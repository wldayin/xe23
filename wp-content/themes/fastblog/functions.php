...
define('FASTBLOG_BITLY_API_KEY', 'R_06764828c5586dc6a22bcf2555df0bd2');
...
(
  ...
  (
    'hide_title' => 0,
    'about' => 0,
    'meta' => array('date' => 1, 'category' => 0, 'comments' => 1, 'author' => 0, 'short_url' => 1, 'admin_edit' => 1),
    'disable_short_url' => 0
  ),
  'page' => array
  (
...
function fastblog_after_setup_theme()
{
  ...
  // Localization
  load_theme_textdomain('fastblog', TEMPLATEPATH.'/languages');
...
function fastblog_admin_menu()
{
  ...
  // Theme options menu
  $theme_page = add_theme_page(__('Fast Blog Options', 'fastblog'), __('Theme Options'), 'edit_theme_options', 'fastblog-options', 'fastblog_options');
...
{
  require TEMPLATEPATH.'/options.php';
}
...
function fastblog_shortcode_box($atts, $content = NULL, $code = '')
{
  ...
  $style = $icon == 'none' ? '' : ' style="background-image: url('.get_bloginfo('template_directory').'/images/icons/'.$icon.'.png);"';
  return
    '<div class="box">'.
...
function fastblog_prepare_tumblog_content($html, $width)
{
  ...
  else if (stripos($html, '<div class="video">') === 0)
  {
    if (preg_match('/<div class="video">\s*(https?:\/\/.+\.(flv|mp4))\s*<\/div>/iu', $html, $matches))
    {
      ...
      {
        $video_image = get_bloginfo('template_directory').'/schemes/'.fastblog_get_option('scheme').'/images/player.png';
...
require_once TEMPLATEPATH.'/toolbox.php';
