...
<?php
  global $authordata;
  $meta = fastblog_get_option( ( is_page() ? 'page' : 'post' ).'/meta' );
?>
...
<div class="meta">
  ...
  <?php if ( isset( $meta['category'] ) && $meta['category'] ): ?>
    ...
    ?>
      <a href="<?php echo get_category_link( $category->term_id ); ?>" class="category" rel="category" title="<?php echo esc_attr( sprintf( __( 'View all posts in %s' ), $category->name ) ); ?>"><?php echo $category->name; ?></a>
    <?php else: ?>
      <span class="category"><?php the_category( ', ' ); ?></span>
    <?php endif; ?>
  <?php endif; ?>
  <?php if ( $meta['comments'] && ( comments_open() || have_comments() ) ): ?>
    <a href="<?php comments_link(); ?>" title="<?php if ( ! ( defined( 'DISQUS_VERSION' ) || defined( 'ID_PLUGIN_VERSION' ) || function_exists( 'livefyre_comments_number' ) ) ) comments_number(); ?>" class="comments"><?php comments_number(); ?></a>
  <?php endif; ?>
