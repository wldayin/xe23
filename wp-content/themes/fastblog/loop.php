...
<?php while ( have_posts() ): the_post(); ?>
  ...
  <div id="post-<?php the_ID(); ?>" <?php post_class( 'post' ); ?>>
    ...
    ?>
      <span class="post-icon <?php echo is_attachment() ? 'attachment' : 'custom'; ?>">
        ...
        ?>
          <a href="<?php echo get_category_link( $category->term_id ); ?>" rel="category" title="<?php echo esc_attr( sprintf( __( 'View all posts in %s' ), $category->name ) ); ?>"><?php _e( 'Post', 'fastblog' ); ?></a>
        <?php endif; ?>
      </span>
    <?php
        else:
  ...
  <?php if ( is_single() ) the_tags( '<div class="tags">', '', '</div>' ); ?>
