...
switch ($_POST['action']) {
    ...
    case 3 :
        ...
        break;
    case 4 :
        MicroblogOption::updateAccount($mid, array( 'suspend' => -1, 'lasttime' => NULL ) );
        $normal = MicroblogOption::getNormalAccountList();
        $wpdb->query("DELETE FROM {$table_prefix}wm_timeline" . ( $normal ? " WHERE mid NOT IN (" . implode(', ', $normal) . ")" : "") );
        wm_delete_cache($mid);
    ...
    switch ($authtype) {
        case 1 :
            ...
            if (!isset($msg['timeline'][0]))
                wm_exit(401, '不能获得用户信息，请检查此微博的隐私设置。');
            if ($action == 1) {
                $return = MicroblogOption::addAccount(array (
                    ...
                    'name' => $msg['user']['name'],
                    'nick' => $msg['user']['nick'],
                    'descrption' => $msg['user']['descrption'],
                    'head' => $msg['user']['head'],
                    'suspend' => FALSE,
                    'authtype' => 1,
                    'token' => NULL,
